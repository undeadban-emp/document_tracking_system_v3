...
function secondsToTime($seconds_time)
{
    if($seconds_time != ''){
        ...
        if ($seconds_time < 365 * 24 * 60 * 60) {
            ...
            }else if($seconds_time >= 3601 && $seconds_time <= 86400){
                return "Duration: ".gmdate('H', $seconds_time).'h:'.gmdate('i', $seconds_time).'m:'.gmdate('s', $seconds_time).'s';
            }else{
                return "Duration: ".gmdate('d', $seconds_time).'d:'.gmdate('H', $seconds_time).'h:'.gmdate('i', $seconds_time).'m:'.gmdate('s', $seconds_time).'s';
            }
        } else {
